...
Contributing
------------
Contributions are welcome. Please open issues for bugs or feature requests. For code changes, provide small pull requests with a concise description and tests where appropriate.

License
-------
This repository does not contain a license file. Add a LICENSE file (e.g., MIT) if you want to make the project open source.

---

If you want, I can also:
- Replace hardcoded API keys with `os.getenv('GENAI_API_KEY')` across the project (automatically),
- Add `fastapi` and `uvicorn` to `requirements.txt`,
- Create a small verification script that checks the environment and prints the resolved API key status (without echoing the key).

Tell me which of these follow-ups you'd like me to perform and I will apply them and validate the workspace.
